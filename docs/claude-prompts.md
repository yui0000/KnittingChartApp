...
### 12) 追加設定。変更箇所。
```
- 行の設定は数値で設定しない。ファイル選択後に、行設定モードになるボタンを活性。
- 行設定モード。ファイル上に半透明黄色の線を表示し、ビンチや移動で線の開始場所、幅を指定、完了で設定済みとする。
```

### 13) 追加設定。変更箇所2。
```
- 行設定モード完了後、行の表示は半透明の黄色。幅も設定したものを保持する。
- 戻るボタンは-にする。機能はカウントを-1するのみ。
- プラスボタン押下で、カウントを+1、行線を上に１行分移動する。
- 行の左端に☑️を配置。
- 行線移動時に☑️を入れる。
- 上から順に☑️はつけ外し可能。チェックを外すと行線も未チェックの部分まで移動する。
```

### 14) 追加設定。変更箇所3。
```
- ☑️は行設定モード完了後に、行幅に合わせて配置する。
- ☑️ボタンをはずしても行番号は変わらないようにする。
- プラスボタン押下で行が動く方向は上。
```
### 15) 追加設定。変更箇所4。
```
- 行とチェックボックスのリセットボタン追加。
- 行設定モードについて。画面上に「行の開始と幅を設定してください」を表示。行の開始位置と幅を設定する。「次へ」ボタン押下、画面上に「行の終了を設定してください」行の終了位置を指定。「完了」ボタンで行設定モードを終了する。
```

### 16) 追加設定。変更箇所5。
```
- 戻るボタンでもカウント−１、行線を下に1行分移動、チェックも外すように変更。
```

### 17) 追加設定。変更箇所6。
```
- プラス、マイナスボタンを真ん中。ペイント、ゴミ箱を右に配置。
```

### 18) 追加設定。変更箇所8。
```
- 行の設定時、次へのボタンの横にキャンセルボタンを設定。
- キャンセルしたら元の画面に戻る。
- 行の設定時、元の設定（行線、チェック）は見えなくする。キャンセルしたら戻せるように保持はする。
- 行の終了は終了位置をクリック（タップ）で指定する。
- 行の終了は開始位置より上にくる。
```

### 19) 追加設定。変更箇所9。
```
- リセットボタン押下後に選択肢を表示「すべて」「行位置のみ」
- 「すべて」はカウント、チェック、行すべてリセットする
- 「行位置のみ」はチェックと行のみリセットする
```
